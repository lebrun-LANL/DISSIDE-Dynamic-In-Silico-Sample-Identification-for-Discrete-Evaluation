...
print("DISSIDE with Raw Data.")

print(paste("Removing samples and features with no observations."))
# Remove 0 rows
feature_raw<-feature_raw[rowSums(feature_raw)>0,]

# Transpose
feature_raw<-as.data.frame(t(feature_raw))
...
if (samples/as.numeric(minSample_inGroup)<2) {
  ...
  # Make relative
  feature_raw<-feature_raw/rowSums(feature_raw)
  ...
  while (!isTRUE(exitcheck)) {
    ...
    if (length(removeout)>=1){
      ...
      feature_out<-feature_raw[removeout,]
      feature_raw<-feature_raw[-removeout,]
    } else {
      exitcheck=TRUE
    }
  }
  ...
  if (any(feature_raw_dist_frame==1)){
    ...
    for (i in seq(1,length(colnames(feature_raw_dist_frame)), by=1)){
      ...
      if (!(list(data) %in% datasets) & (length(data)>1)) {
    ...
    for (i in seq(1,length(datasets), by=1)) {
      new_data<-as.data.frame(t(feature_raw[datasets[[i]],]), drop = FALSE)
      new_data<-new_data[rowSums(new_data)>0,]
    ...
    if (maxGroup=="NULL") {
      ...
      while (groups<=as.numeric(maxGroup))
      {
        if (length(which(table(cutree(feature_raw_clust, k=groups))>=as.numeric(minSample_inGroup)))>1) {
          feature_raw_clust_cut<-as.data.frame(cutree(feature_raw_clust, k=groups))
          colnames(feature_raw_clust_cut)<-"Group"
          group_adonis<- adonis2(feature_raw ~ as.factor(feature_raw_clust_cut$Group),
                                permutations = permutations, method = distmetric, parallel = numcores)
          group_anosim<- anosim(feature_raw, grouping = as.factor(feature_raw_clust_cut$Group),
                                distance = distmetric, permutations = permutations, parallel = numcores)
          if (group_adonis$`Pr(>F)`[1]<=0.05 && group_anosim$signif<=0.05) {
           groupFs<-rbind(groupFs,cbind(group_adonis$F[1],group_adonis$`Pr(>F)`[1], group_anosim$statistic, group_anosim$signif))
          } else {
            groupFs<-rbind(groupFs,rep(NA,4))
    ...
    if (length(rownames(na.omit(groupFs)))==0) {
      ...
      #get an error regarding self-contained argument = T, says it will be deprecated soon so will need to update it for future versions of htmlwidgets
      ...
      if (length(rownames(feature_no_out))==length(rownames(feature_raw))) {
        ...
        # Remove 0 features
        feature_no_out<-feature_no_out[rowSums(feature_no_out)>0,]
        
        # Calculate the centroids of clean version
        #cluster the distances based on optimal group numbers which were previously calculated
